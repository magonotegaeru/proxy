...
class Robot(object):
    ...
    # 自分で作成した
    def do_select_poses(self, posename=select_poses_lines.W0[0]["poses"], duration=1.0,time_weight = 2):
        """
        select_poes_lines.pyに定義されているposeを選択し、そのポーズをする。
        speedはポーズの早さ。msec = 1000/speed
        speedが1.0なら1000msecで動作する。
        """
        map = posename
        msec = int(1000 / duration)*time_weight
        pose = dict(msec=msec, map=map)
        data = json.dumps(pose).encode('utf-8')
        client.send(self.ip, self.pose_port, data)
        return msec
